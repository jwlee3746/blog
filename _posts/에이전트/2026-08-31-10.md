...
| --- | --- | --- |
| **Code-based** | unit test, exact·regex match, schema, static analysis, state check | 빠르고 재현 가능하지만 열린 해법과 주관적 품질에는 지나치게 rigid할 수 있다. |
| **Model-based** | 자연어 품질, 복합 rubric, 열린 형태의 결과와 transcript 분석 | 확장성이 높지만 model bias, 비결정성, 비용을 관리하고 human label과 calibration해야 한다. |
| **Human** | 주관적 품질, 복잡한 정책, domain expertise가 필요한 판정 | 가장 풍부한 판단을 제공하지만 느리고 비싸며 평가자 간 일관성 관리가 필요하다. |
...
| 신호 | 주된 역할 |
| --- | --- |
| **Automated eval** | 일관된 측정, baseline 설정과 배포 전 regression 탐지 |
| **Production monitoring** | 실제 규모에서 distribution drift, 드문 edge case와 시스템 이상 탐지 |
| **A/B testing** | 충분한 traffic에서 중요한 변경이 실제 사용자 결과를 개선하는지 검증 |
| **User feedback** | 자동 기준에 없던 실패와 product expectation 발견 |
| **Transcript review·early access** | 미묘한 failure와 예상 밖의 사용자·agent behavior 발견 |
| **Systematic human study** | 주관적 출력 평가와 model grader calibration의 기준 제공 |
