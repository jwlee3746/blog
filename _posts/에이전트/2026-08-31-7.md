...
> 이 비교는 설명을 위해 일반화한 것이다. 특정 제품의 evaluator, task 수, 모델 또는 운영 기준을 나타내지 않는다.

[시리즈 허브]({{site.url}}{{site.baseurl}}/Agent/reproducible-agent-evaluation/) · 본편 2/4

## 잘못 세운 대립부터 수정하기

처음에는 TC 기반과 simulator 기반을 서로 대체하는 평가 방식으로 생각하기 쉽다. 하지만 공개된 최신 harness와 benchmark를 보면 simulator 평가에도 task가 있다. Instruction, 초기 database 상태, 사용 가능한 tool, policy와 성공 기준을 정하고 그 안에서 user simulator와 agent가 동적으로 대화한다.
...
다음: [3. 최신 evaluation harness의 공통 구조]({{site.url}}{{site.baseurl}}/Agent/agent-evaluation-harness-landscape/)
