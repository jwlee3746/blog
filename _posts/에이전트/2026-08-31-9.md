...
TC 중심 평가를 설계한 선택은 재현성, 회귀 검증과 디버깅이라는 분명한 문제를 해결했다. Schema와 validation boundary를 세우고, multi-step history에서 기대값을 분리하고, deterministic scoring과 checkpoint를 만든 일도 지금의 harness 구조와 어긋나지 않는다.
