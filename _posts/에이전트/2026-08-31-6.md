...
> 이 글은 공개 표준과 도구를 기준으로 정리한 일반화된 설계다. 실제 제품의 trace schema, 저장소, 데이터 규모와 운영 설정을 나타내지 않는다.
...
Langfuse는 trace나 observation에서 dataset item을 만들고 dataset run과 score를 연결하는 기능을 제공한다. LangSmith도 production run을 filter해 dataset으로 보내고, offline evaluation 결과를 다시 online monitoring에 연결하는 흐름을 문서화한다. 중요한 점은 버튼 자체가 아니라 그 사이에 있는 **승격 정책**이다.
...
OpenTelemetry의 GenAI semantic conventions는 공통 언어를 제공하지만 계속 발전하는 영역이다. 따라서 collector와 저장소에서 사용하는 convention version을 기록하고, 제품 고유 attribute는 별도 namespace로 격리하는 편이 안전하다.
