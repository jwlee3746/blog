---
title: "[Agent Eval 3] 최신 Evaluation Harness는 무엇을 표준화하는가"
excerpt: "Anthropic의 개념 모델, Inspect AI, Harbor와 τ-bench를 비교해 agent evaluation harness의 공통 구조를 정리한다."
...
> 공개 문서와 논문을 기준으로 공통점을 추린 reference model이다. 특정 framework의 완전한 기능 비교나 실제 제품 architecture를 나타내지 않는다.
...
| Inspect AI | dataset sample을 가진 Task | solver/agent, tool, sandbox | scorer, epochs, logs와 checkpoint |
| Harbor | instruction·environment·test를 가진 Task | containerized environment | trial과 test script, job 단위 실행 |
...
4. Trace와 artifact를 외부 관측 도구에 연결할 수 있는가?
5. Infrastructure failure를 평가 실패와 분리하는가?
