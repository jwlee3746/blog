---
title: "[Agent Evaluation 4] 장시간 평가의 복구와 provenance"
...
> 이 글은 장시간 batch evaluation에 적용할 수 있는 일반화된 운영 설계를 설명한다. 다이어그램의 component, 주기와 저장 방식은 실제 프로덕션 설정을 나타내지 않는다.

[시리즈 허브]({{site.url}}{{site.baseurl}}/Agent/reproducible-agent-evaluation/) · 4/4
...
Continuation이나 retry에도 별도 budget을 둔다. 그렇지 않으면 case 하나가 반복 discovery나 transient error로 전체 run capacity를 점유할 수 있다.
...
이전: [3. 경로 채점과 discovery continuation]({{site.url}}{{site.baseurl}}/Agent/agent-path-scoring/)
